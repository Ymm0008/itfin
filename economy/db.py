...
def get(table1,table2,table3,table4,field):
	...
	conn.autocommit(True)
	cur = conn.cursor()
	sql1 = "select el.id,el.entity_name,el.entity_type,el.regist_address,pd.operation_mode from %s as el inner join %s as pd on el.id=pd.entity_id and pd.date='2017-11-27'" % (table1,table2)
	cur.execute(sql1)
	res1 = cur.fetchall()
	sql2 = "select el.id,el.entity_name,el.entity_type,el.regist_address,cd.operation_mode from %s as el inner join %s as cd on el.id=cd.entity_id and cd.date='2017-11-27'" % (table1,table3)
	cur.execute(sql2)
	res2 = cur.fetchall()
	sql3 = "select el.id,el.entity_name,el.entity_type,el.regist_address,p.operation_mode from %s as el inner join %s as p on el.id=p.entity_id and p.date='2017-11-27'" % (table1,table4)
	cur.execute(sql3)
	res3 = cur.fetchall()
	res = res1 + res2 + res3
	if res:
